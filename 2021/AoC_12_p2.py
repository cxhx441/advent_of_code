    ...
    def get_all_paths_helper(self, start, end, visited, cur_path, all_paths):
        ...
        if start == end: 
            ...
            for vertex in self.neighbors[start]:
                # get number of small caves that have been visited more than once
                small_cave_visited_more_than_once_count = 0
                for key in visited:
                    if key == key.lower() and visited[key] > 1:
                        small_cave_visited_more_than_once_count += 1
                if visited[vertex] == 0 or vertex.upper() == vertex or (small_cave_visited_more_than_once_count == 0 and vertex not in ('start', 'end')):
